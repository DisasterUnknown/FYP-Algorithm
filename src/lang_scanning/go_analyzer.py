...
class GoAnalyzer:
    ...
    def _go_module_path(self):
        root = walk_up_for_marker(Path(self.file_path), "go.mod")
        if root is None:
            return None
        try:
    ...
    def _resolve_go_import(self, import_path):
        ...
        return first_existing([package_dir.with_suffix(".go")])
        ...
        for line in self.get_import_list():
            for import_path in quoted_paths_from_import_line(line):
                append_graph_edge(
                    edges,
                    seen,
                    self._resolve_go_import(import_path),
                    self.file_path,
                )
        return edges
