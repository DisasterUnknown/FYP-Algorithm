...
class CSharpAnalyzer:
    ...
    def _resolve_csharp_using(self, line):
        match = re.search(r"using\s+(?:static\s+)?([\w.]+)", line)
        if not match:
            return None

        namespace = match.group(1).rstrip(";")
        project_root = walk_up_for_marker(Path(self.file_path), ".csproj")
        if project_root is None:
            return None

        type_name = namespace.split(".")[-1]
        for cs_file in project_root.rglob("*.cs"):
            if cs_file.name == f"{type_name}.cs":
                try:
                    content = cs_file.read_text(encoding="utf-8", errors="ignore")
                except OSError:
                    continue
                if f"namespace {namespace}" in content or f"namespace {namespace};" in content:
                    return cs_file
        return None
    ...
    def get_graph_incoming_edges(self):
        ...
        for line in self.get_import_list():
            append_graph_edge(
                edges,
                seen,
                self._resolve_csharp_using(line),
                self.file_path,
            )
        return edges
