import re
from pathlib import Path
from tree_sitter import Language, Parser
from src.utils.import_graph_utils import (
    append_graph_edge,
    first_existing,
    quoted_paths_from_import_line,
    walk_up_for_marker,
)


class DartAnalyzer:
    def __init__(self, code_path: str):
        self.file_path = code_path
        self.source_code = self._load_source(code_path)

        self.language = Language('build/languages.so', "dart")
        self.parser = Parser()
        self.parser.set_language(self.language)

        self.tree = self._parse(self.source_code)

    # ==================================================
    # core helpers
    def _parse(self, source_code: str):
        return self.parser.parse(bytes(source_code, "utf8"))

    def _walk(self, node):
        yield node
        for child in node.children:
            yield from self._walk(child)

    def _text(self, node, source_code: str) -> str:
        return source_code[node.start_byte:node.end_byte]

    def _load_source(self, file_path: str) -> str:
        with open(file_path, "r", encoding="utf-8") as f:
            return f.read()

    # ==================================================
    # imports
    def get_import_list(self):
        imports = []
        seen = set()
        for node in self._walk(self.tree.root_node):
            if node.type == "import_or_export":
                text = self._text(node, self.source_code).strip()
                if text.startswith("import") and text not in seen:
                    seen.add(text)
                    imports.append(text)

        return imports

    def get_number_of_imports(self):
        return len(self.get_import_list())

    def _dart_project_root(self):
        return walk_up_for_marker(Path(self.file_path), "pubspec.yaml")

    def _dart_package_name(self, project_root):
        pubspec = project_root / "pubspec.yaml"
        try:
            text = pubspec.read_text(encoding="utf-8", errors="ignore")
        except OSError:
            return None
        match = re.search(r"^name:\s*(\S+)", text, re.MULTILINE)
        return match.group(1) if match else None

    def _resolve_dart_import(self, import_path):
        if import_path.startswith("dart:"):
            return None

        current_dir = Path(self.file_path).parent

        if import_path.startswith("package:"):
            project_root = self._dart_project_root()
            if project_root is None:
                return None
            rest = import_path[len("package:"):]
            if "/" not in rest:
                return None
            package_name, subpath = rest.split("/", 1)
            local_name = self._dart_package_name(project_root)
            if not local_name or package_name != local_name:
                return None
            candidate = project_root / "lib" / subpath
            return first_existing([candidate])

        candidate = (current_dir / import_path).resolve()
        return first_existing([candidate])

    def get_graph_incoming_edges(self):
        edges = []
        seen = set()
        for line in self.get_import_list():
            for import_path in quoted_paths_from_import_line(line):
                append_graph_edge(
                    edges,
                    seen,
                    self._resolve_dart_import(import_path),
                    self.file_path,
                )
        return edges

    # ==================================================
    # classes
    def get_class_list(self):
        classes = []
        seen = set()
        for node in self._walk(self.tree.root_node):
            if node.type == "class_definition":
                text = self._text(node, self.source_code).strip()
                first_line = text.split("{")[0].strip()
                if first_line not in seen:
                    seen.add(first_line)
                    classes.append(first_line)

        return classes

    def get_number_of_classes(self):
        return len(self.get_class_list())

    # ==================================================
    # functions / methods
    def get_function_list(self):
        functions = []
        seen = set()
        for node in self._walk(self.tree.root_node):
            if node.type in [
                "function_signature",
                "method_signature",
                "function_declaration",
                "method_declaration"
            ]:
                text = self._text(node, self.source_code).strip()
                if "(" in text:
                    name = text.split("(")[0].strip()
                    if name not in seen:
                        seen.add(name)
                        functions.append(name)

        return functions

    def get_number_of_functions(self):
        return len(self.get_function_list())

    # ==================================================
    # comments
    def get_comment_lines(self):
        lines = set()
        for node in self._walk(self.tree.root_node):
            if "comment" in node.type:
                start_line = node.start_point[0]
                end_line = node.end_point[0]
                for i in range(start_line, end_line + 1):
                    lines.add(i)

        return len(lines)

    # ==================================================
    # crypto detection
    def get_contains_crypto_usage(self):
        crypto_imports = [
            "package:crypto",
            "pointycastle",
            "crypto"
        ]
        crypto_calls = [
            "sha256",
            "sha1",
            "md5",
            "encrypt",
            "decrypt",
            "hmac",
            "aes"
        ]

        # check imports
        for node in self._walk(self.tree.root_node):
            if node.type == "import_or_export":
                text = self._text(node, self.source_code).lower()
                if any(i in text for i in crypto_imports):
                    return True

        # check actual usage
        for node in self._walk(self.tree.root_node):
            text = self._text(node, self.source_code).lower()
            if any(c in text for c in crypto_calls):
                return True

        return False

    def get_crypto_algorithms(self):
        crypto_algorithms = {
            "sha1", "sha256", "sha512", "md5",
            "aes", "rsa", "ecdsa", "hmac"
        }

        found = set()
        for node in self._walk(self.tree.root_node):
            text = self._text(node, self.source_code).lower()
            # ONLY accept real usage contexts
            if node.type in [
                "constructor_invocation",
                "argument_part",
                "identifier",
            ]:
                for algo in crypto_algorithms:
                    if algo in text:
                        found.add(algo)

        return list(found)

    # ==================================================
    # database detection
    def get_contains_database_access(self):
        db_imports = [
            "sqflite",
            "drift",
            "hive",
            "isar",
            "sqlite"
        ]
        db_usage_patterns = [
            ".insert",
            ".query",
            ".delete",
            ".update",
            "openbox",
            "db.",
            "database"
        ]

        for node in self._walk(self.tree.root_node):
            if node.type == "import_or_export":
                text = self._text(node, self.source_code).lower()
                if any(d in text for d in db_imports):
                    return True

        for node in self._walk(self.tree.root_node):
            text = self._text(node, self.source_code).lower()
            if any(p in text for p in db_usage_patterns):
                return True

        return False

    def get_database_tables(self):
        known_tables = [
            "users",
            "transactions",
            "budget",
            "categories",
            "settings"
        ]
        db_operations = [
            "insert",
            "query",
            "update",
            "delete",
            "select",
            "where",
            "from"
        ]

        found = set()
        for node in self._walk(self.tree.root_node):
            if node.type in [
                "constructor_invocation",
                "argument_part",
                "identifier",
            ]:
                text = self._text(node, self.source_code).lower()
                
                # must be in DB context first
                if any(op in text for op in db_operations):        
                    for table in known_tables:
                        if table in text:
                            found.add(table)

        return list(found)

    # ==================================================
    # file access detection
    def get_contains_file_access(self):
        for node in self._walk(self.tree.root_node):
            if node.type == "import_or_export":
                text = self._text(node, self.source_code).lower()
                if "dart:io" in text:
                    return True
    
        file_patterns = [
            "file(",
            "readasstring",
            "writeasstring",
            "directory",
            "path_provider"
        ]
        for node in self._walk(self.tree.root_node):
            text = self._text(node, self.source_code).lower()
            if any(p in text for p in file_patterns):
                return True
    
        return False

    # ==================================================
    # network access detection
    def get_contains_network_access(self):
        network_imports = [
            "package:http",
            "dio",
            "websocket"
        ]
        for node in self._walk(self.tree.root_node):
            if node.type == "import_or_export":
                text = self._text(node, self.source_code).lower()
                if any(n in text for n in network_imports):
                    return True

        network_calls = [
            "get(",
            "post(",
            "put(",
            "delete(",
            "dio.",
            "http."
        ]
        for node in self._walk(self.tree.root_node):
            text = self._text(node, self.source_code).lower()
            if any(c in text for c in network_calls):
                return True

        return False

    # ==================================================
    # auth usage detection
    def get_contains_auth_usage(self):
        for node in self._walk(self.tree.root_node):
            if node.type == "import_or_export":
                text = self._text(node, self.source_code).lower()
                if "firebaseauth" in text:
                    return True

        auth_patterns = [
            "signin",
            "signup",
            "auth.",
            "token",
            "jwt"
        ]
        for node in self._walk(self.tree.root_node):
            text = self._text(node, self.source_code).lower()
            if any(p in text for p in auth_patterns):
                return True

        return False

    # ==================================================
    # backuplogic detection
    def get_contains_backup_logic(self):
        backup_imports = [
            "backup_service",
            "cloud_firestore",
            "shared_preferences"
        ]
        backup_patterns = [
            "backup",
            "restore",
            "export",
            "importdata",
            "sync"
        ]

        for node in self._walk(self.tree.root_node):
            if node.type == "import_or_export":
                text = self._text(node, self.source_code).lower()
                if any(b in text for b in backup_imports):
                    return True

        for node in self._walk(self.tree.root_node):
            text = self._text(node, self.source_code).lower()
            if any(p in text for p in backup_patterns):
                return True

        return False