...
class KotlinAnalyzer:
    def __init__(self, code_path: str):
        ...
        self.parser, self.language = create_parser_with_language("kotlin")
    ...
    def _resolve_kotlin_import(self, line):
        ...
        rel = Path(*qualified.split(".")).with_suffix(".kt")
    ...
    def get_graph_incoming_edges(self):
        ...
        for line in self.get_import_list():
            append_graph_edge(
                edges,
                seen,
                self._resolve_kotlin_import(line),
                self.file_path,
            )
        return edges
    ...
    def get_function_list(self):
        ...
        for node in self._walk(self.tree.root_node()):
            if node.kind() != "function_declaration":
                continue
            name = name_from_definition(
                node, self.source_code, identifier_types=("simple_identifier", "identifier")
            )
